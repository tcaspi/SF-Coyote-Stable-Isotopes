...
species.names <- c("ff_chicken" = "Chicken",
          "wet.food"= "Pet Food",
          "medium.mammal"= "Medium Mammals", 
          "rat"= "Rats",
          "gopher"= "Gophers",
          "fruit"= "Fruit")
...
These models take a long time to run - it is recommended to run them on an HPC cluster.
...
species_stats <- prey %>%
  ...
  # select cols
  select(c(SampleID, species, d13C, d15N)) %>% 
...
# ggsave("Figures/Figure5a.png", dpi=600, height=3, width=4)
```
...
# df.stats <- readRDS("Model_Output/df")
# posterior_global <- readRDS("Model_Output/posterior_global")
```
...
ind.props_plotting_all <- ind.props %>% 
  ...
  left_join(data_averages %>% select(c(whisker, group, individual, sfcoy)) , by="whisker") %>% 
  group_by(whisker) %>% 
  mutate(Proportion = Proportion / sum(Proportion)) %>% 
  ungroup() %>% 
  left_join(covs, by="group") %>% 
  mutate(group = factor(group, levels = c("Coit", "Bernal", "St. Francis", "Glen", "GGP - East", "GGP - West", "McLaren", "Merced", "Presidio", NA))) %>% 
...
whisker_to_sfcoy <- ind.props_plotting_all %>%
  select(whisker, sfcoy) %>%
  distinct() %>%
  deframe()


# source labels
mean.names <- c("Chicken", "Pet Food", "Medium-Sized \nMammals", "Rats", "Fruit", "Gophers")

# plot
fig.5b <- ind.props_plotting_all %>%
  ggplot(aes(x=reorder(whisker, group), y=Proportion, fill=Source)) +
  geom_col()+
  ...
  scale_x_discrete(labels = whisker_to_sfcoy)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1.15, hjust=1, size=3),
        axis.ticks.x = element_blank(),
        strip.text.x = element_text(size = 3, angle = 0, hjust=0.5, vjust=.5),
...
# ggsave("Figures/Figure5b.png", dpi=600, height=4, width=8)
```
...
consumer_df_isa <- data.frame(
  ...
  individual = data_averages_isa$whisker,
  group = data_averages_isa$group) %>% 
  mutate_if(is.character, as.factor) 
...
ggplot()+
  ...
  geom_point(data=consumer_df_avgs, mapping=aes(x=d13C, y=d15N))+
  theme_custom()+
  scale_y_continuous(limits=c(4,13), breaks = seq(4, 15, by = 1))+
...
```{r}
mix.coy.isa # same as previous model with six diet sources

source.coy.three <- load_source_data(filename="Data/Output/coyote_source_three.csv",
                           source_factors=NULL, 
                           conc_dep=FALSE, 
                           data_type="means", 
                           mix.coy.avgs)

discr.coy.three <- load_discr_data(filename="Data/Output/coyote_discrimination_three.csv", mix.coy.avgs)
```
