...
# Calculate SEAb values for each individual in each population
SEA.B.all.inds <- siberEllipses(ellipses.posterior.inds)

names_df <- siber_df %>% distinct(individual,urban) %>% arrange(urban) # extract id names

names <- names_df$individual

colnames(SEA.B.all.inds) <- names # add id names
...
results_df <- data.frame(individual = extract_vals$sampleID,
  ...
  left_join(data_clean %>% select(sex, individual, urban) %>% distinct()) # add sex/site info

# Calculate mean SEAb and standard errors for individuals in SF vs Marin
results_df %>% 
  group_by(urban) %>% 
...
ggsave("Figures/Figure3.png",dpi=600, width=8, height=4)
```
